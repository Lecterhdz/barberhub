// src/views/admin/inventario/inventario.js
...
async function guardarProductos() {
    ...
    if (window.app && window.app.estado) {
        window.app.estado.cache.productos = productos;
    }
}
...
function renderizarTabla() {
    ...
    tbody.innerHTML = paginated.map(producto => {
        ...
        return `
            <tr>
                <td onclick="verProducto(${producto.id})">
                    <div>
                        <div style="font-weight: 600;">${escapeHtml(producto.nombre)}</div>
                        ${producto.descripcion ? `<div style="font-size: 0.75rem; color: var(--text-secondary);">${escapeHtml(producto.descripcion.substring(0, 40))}</div>` : ''}
                    </div>
                </td>
                <td onclick="verProducto(${producto.id})">${producto.categoria}</td>
                <td onclick="verProducto(${producto.id})">$${producto.precio.toLocaleString()}</td>
                <td onclick="verProducto(${producto.id})">$${producto.costo.toLocaleString()}</td>
                <td onclick="verProducto(${producto.id})"><span class="stock-badge ${stockClass}">${stockText}</span></td>
                <td class="${gananciaClass}" onclick="verProducto(${producto.id})">$${ganancia.toLocaleString()}</td>
...
window.verProducto = function(id) {
    ...
    detalle.innerHTML = `
        <div class="info-row"><label>Nombre:</label><span>${escapeHtml(producto.nombre)}</span></div>
        <div class="info-row"><label>Categoría:</label><span>${producto.categoria}</span></div>
        <div class="info-row"><label>Precio venta:</label><span>$${producto.precio.toLocaleString()}</span></div>
        <div class="info-row"><label>Costo:</label><span>$${producto.costo.toLocaleString()}</span></div>
        <div class="info-row"><label>Ganancia:</label><span class="${ganancia > 0 ? 'ganancia-positiva' : 'ganancia-negativa'}">$${ganancia.toLocaleString()} (${margen}%)</span></div>
        <div class="info-row"><label>Stock:</label><span>${producto.stock} / ${producto.stockMinimo} mínimo</span></div>
        <div class="info-row"><label>Proveedor:</label><span>${escapeHtml(producto.proveedor || 'No especificado')}</span></div>
        ${producto.descripcion ? `<div class="info-row"><label>Descripción:</label><span>${escapeHtml(producto.descripcion)}</span></div>` : ''}
    `;
...
window.eliminarProducto = async function(id) {
    if (confirm('¿Eliminar este producto?')) {
        ...
        renderizarTabla();
        verificarAlertasStock();
        window.app?.mostrarNotificacion('Producto eliminado', 'success');
    }
};
...
async function guardarProducto(e) {
    ...
    if (editingId) {
        ...
        window.app?.mostrarNotificacion('Producto actualizado', 'success');
    } else {
        productoData.id = Date.now();
        productos.push(productoData);
        window.app?.mostrarNotificacion('Producto agregado', 'success');
    }
...
function aplicarAjusteStock() {
    ...
    if (!cantidad || cantidad <= 0) {
        window.app?.mostrarNotificacion('Ingrese una cantidad válida', 'warning');
        return;
    }
    
    if (tipo === 'entrada') {
        producto.stock += cantidad;
        window.app?.mostrarNotificacion(`Se agregaron ${cantidad} unidades`, 'success');
    } else {
        if (producto.stock - cantidad < 0) {
            window.app?.mostrarNotificacion('No hay suficiente stock', 'error');
            return;
        }
        producto.stock -= cantidad;
        window.app?.mostrarNotificacion(`Se retiraron ${cantidad} unidades`, 'warning');
    }
...
function setupModalClose() {
    ...
    if (stockModal) {
        ...
        if (closeBtn) closeBtn.onclick = () => stockModal.style.display = 'none';
        stockModal.onclick = (e) => { if (e.target === stockModal) stockModal.style.display = 'none'; };
    }
}
