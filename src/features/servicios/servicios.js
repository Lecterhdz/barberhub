// src/features/servicios/servicios.js

console.log('✂️ Servicios feature cargado');
...
// Inicializar
async function init() {
    console.log('✂️ Inicializando gestión de servicios...');
    await cargarDatos();
    setupEventListeners();
    renderizarGrid();
    setupModalClose();
...
function getServiciosEjemplo() {
    return [
        ...
        { id: 5, nombre: 'Tratamiento Capilar', categoria: 'Tratamiento', precio: 400, duracion: 45, estado: 'activo', icono: '💆', descripcion: 'Hidratación y nutrición profunda', barberos: [1, 3] }
    ];
}
    ...
    grid.innerHTML = paginated.map(servicio => `
        <div class="servicio-card">
            ...
            <div class="servicio-card-header">
                ...
                <h3 class="servicio-nombre-card">${servicio.nombre}</h3>
...
window.eliminarServicio = async function(id) {
    const confirmar = await window.utils?.confirmar('¿Eliminar este servicio?');
    if (confirmar) {
        servicios = servicios.filter(s => s.id !== id);
        await guardarServicios();
        renderizarGrid();
        window.utils?.mostrarNotificacion('Servicio eliminado', 'success');
    }
};
...
async function guardarServicio(event) {
    ...
    if (editingId) {
        ...
        window.utils?.mostrarNotificacion('Servicio actualizado', 'success');
    } else {
...
// Cerrar modal
function cerrarModal(modalId = 'servicio-modal') {
